...
if ($_SESSION['adminKind'] == 1) {
	...
	//否则只能查看自己栏目的文章 
	$where = ' where tkind='.((int)$_SESSION['adminKind']);
	if ($_SESSION['adminKind'] == 3) {
		$where .= " or tkind=".((int)$_SESSION['adminKind'] + 1);
	}
}
...
$sql = "select * from article".$where." {$orderBy} limit {$offset},{$pageSize}";
$rows = fetchAll($sql);
?>
<!doctype html>
<html>
...
<div class="details">
                    ...
                    <!--表格-->
               		<?php 
                        	if (!$rows) {
                        		echo "没有文章!请<br/><a href='addArticle.php'>添加文章</a>";
                        	} else {
                        		?>
                    <table class="table" cellspacing="0" cellpadding="0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td align="center">
                                				<input type="button" value="详情" class="btn" onclick="showDetail(<?php echo $row['id'];?>,'<?php echo $row['title'];?>')"><input onclick="editArticle(<?php echo $row['id'];?>)" type="button" value="修改" class="btn"  >
                                				<input onclick="delArticle(<?php echo $row['id'];?>)" type="button" value="删除" class="btn">
...
<script type="text/javascript">
	...
	function change(val){
		window.location="listArticle.php?order="+val;
	}
</script>
</html>
